...
const CourseList = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingCourseId) {
                await axios.put(`http://localhost:8080/api/courses/${editingCourseId}`, newCourse);
        ...
        <Container fluid>
                                ...
                                <tbody>
                                    {currentCourses.map((course, index) => (
                                        <tr key={course.id}>
                                            ...
                                            <td>
                                                {course.activate ? <Badge variant="success">Đang Mở</Badge> : <Badge variant="danger">Đã Đóng</Badge>}
                                            </td>
                                            <td>
                                                <Button variant="info" className="btn-action" onClick={() => handleEdit(course.id)}>
                                                    Sửa
                                                </Button>
            ...
            <Modal show={showModal} onHide={resetForm} size="lg" centered>
                <Modal.Header closeButton>
                    <Modal.Title>{editingCourseId ? 'Cập Nhật Khóa Học' : 'Thêm Khóa Học'}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form onSubmit={handleSubmit}>
                        ...
                        <Form.Group controlId="formActivate">
                            <Form.Check
                                ...
                                onChange={() => setNewCourse({ ...newCourse, activate: !newCourse.activate })}
                            />
                        </Form.Group>
                        <Modal.Footer>

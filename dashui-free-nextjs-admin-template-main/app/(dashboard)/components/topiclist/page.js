...
import { Badge, Button, Card, Table, Container, Row, Col, Form, Modal, Pagination } from "react-bootstrap";
...
function TopicList() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingTopicId) {
        await axios.put(`http://localhost:8080/api/topics/${editingTopicId}`, { ...newTopic });
    ...
    <Container fluid>
          ...
          <Card className="strpied-tabled-with-hover">
            <Card.Header>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                  <Button variant="primary" onClick={() => setShowTuitionFeeList(true)}>
                      Chọn Học Phí
                  </Button>
                ...
                <tbody>
                  {currentTopics.map((topic, index) => (
                    <tr key={topic.id}>
                      ...
                      <td>
                      <Button variant="info" className="btn-action" onClick={() => handleEdit(topic.id)}>
                        Sửa
                      </Button>
      ...
      <Modal show={showForm} onHide={handleCloseModal} size="lg" centered>
        <Modal.Header closeButton>
          <Modal.Title>{editingTopicId ? 'Cập Nhật Chuyên Đề' : 'Thêm Chuyên Đề'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form onSubmit={handleSubmit}>
            <Form.Group controlId="formTopicsName">
              <Form.Label>Tên Chuyên Đề</Form.Label>
              <Form.Control type="text" name="topicName" value={newTopic.topicName} onChange={handleInputChange} required />
            </Form.Group>
            <Form.Group controlId="formTopicsCode">
              <Form.Label>Mã Chuyên Đề</Form.Label>
              <Form.Control type="text" name="topicCode" value={newTopic.topicCode} onChange={handleInputChange} required />
            </Form.Group>
            <Form.Group controlId="formTheoryHours">
              <Form.Label>Giờ Lý Thuyết</Form.Label>
              <Form.Control type="number" name="theoryHours" value={newTopic.theoryHours} onChange={handleInputChange} required />
            </Form.Group>
            <Form.Group controlId="formPracticalHours">
              <Form.Label>Giờ Thực Hành</Form.Label>
              <Form.Control type="number" name="practicalHours" value={newTopic.practicalHours} onChange={handleInputChange} required />
            </Form.Group>
            <Form.Group controlId="formOriginalPrice">
              <Form.Label>Giá Gốc</Form.Label>
              <Form.Control type="number" name="originalPrice" value={newTopic.originalPrice} onChange={handleInputChange} required />
            </Form.Group>
            <Form.Group controlId="formPromotionalPrice">
              <Form.Label>Giá Khuyến Mãi</Form.Label>
              <Form.Control type="number" name="promotionalPrice" value={newTopic.promotionalPrice} onChange={handleInputChange} required />
            </Form.Group>

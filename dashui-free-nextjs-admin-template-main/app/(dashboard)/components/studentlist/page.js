...
import { Button, Card, Table, Container, Row, Col, Alert, Spinner, Pagination, Modal, Form } from "react-bootstrap";
...
function StudentList() {
  ...
  const handleSave = async () => {
    try {
      await axios.put(`http://localhost:8080/api/student/${selectedStudent.id}`, selectedStudent);
      fetchStudents();
      setShowModal(false);
                  ...
                  {currentStudents.length > 0 ? (
                    currentStudents.map((student, index) => (
                      <tr key={student.id}>
                        ...
                        <td>{student.activate ? 'Còn Học' : 'Đã Tốt Nghiệp'}</td>
                        <td>
                          <Button
                            variant="info"
      ...
      {selectedStudent && (
        <Modal show={showModal} onHide={() => setShowModal(false)} centered>
          <Modal.Header closeButton>
            <Modal.Title>{editMode ? "Chỉnh sửa thông tin học viên" : "Thông tin chi tiết học viên"}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            {editMode ? (
              <Form>
                ...
                <Form.Group>
                  <Form.Label>Loại Học Viên</Form.Label>
                  <Form.Control
                    type="text"
                    name="studentTypeName"
                    value={selectedStudent.studentTypeName || ''}
                    onChange={handleInputChange}
                  />
                </Form.Group>
                <Form.Group>
                  ...
                  <Form.Control
                    type="text"
                    name="activate"
                    value={selectedStudent.activate || ''}
                    onChange={handleInputChange}
                  />
                </Form.Group>
                <Form.Group>
              ...
              <>
                <p><strong>Tên:</strong> {selectedStudent.fullName}</p>
                <p><strong>Ngày Sinh:</strong> {selectedStudent.dateOfBirth}</p>
                ...
                <p><strong>Mã Học Viên:</strong> {selectedStudent.studentCode}</p>
                <p><strong>Loại Học Viên :</strong> {selectedStudent.studentTypeName}</p>
                <p><strong>Trạng Thái:</strong> {selectedStudent.activate ? 'Hoạt Động' : 'Không Hoạt Động'}</p>
                <p><strong>Ảnh:</strong> {selectedStudent.imageName}</p>
              </>
            )}
          </Modal.Body>
          <Modal.Footer>
